...
$select ="SELECT * From `product` left join `categories` on product.Cat_ID = categories.catID left join `local_brand` on product.lID = local_brand.lID WHERE `sID` is null AND `P_name` LIKE '%$searched%' OR `l_name` LIKE '%$searched%'";
$s =mysqli_query($conn , $select);
if (isset($_GET["search"])) {
    $searched = addSlashes($_GET["searched"]);
    header("location: /printerly/products/searchproduct.php?searched=" . $searched);
}
...
<div class="form-outline">
  ...
  <button type="submit" class="btn btn-primary" style="height:40px; background-color:purple; border:purple;">
