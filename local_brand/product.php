...
if (isset($_GET['pID'])){
    $id=$_GET['pID'];
    $select ="SELECT * From `product` left join `categories` on product.Cat_ID = categories.catID left join `supplier` on product.sID = supplier.sID WHERE pID=$id";
...
$insert = "INSERT INTO `brandsorder` values(NULL ,$selectedquantity,'$selectedsize','$selectedcolor',$orderprice,Default,'$details',$id,$bid)";
...
header("location: /printerly/local_brand/ordersfromsupplier.php");
...
<?php } ?>
                ...
                <p>supplier Name : <?php echo $fetch['s_name']; ?></p>
...
<textarea style="width : 300px; height: 150px;  resize: none; margin : 0 auto ;"  class="form-control" id="exampleFormControlTextarea1" name="details" rows="5"
                                    placeholder="if you have any more details please enter it ......"></textarea>
