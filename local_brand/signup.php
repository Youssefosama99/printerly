  ...
  @$bemail=$fetch['l_email'];
    ...
    elseif($Email == $bemail){
      ...
      header("Refresh:1; url=login.php");  
        ...
        $insert = " INSERT INTO `local_brand` VALUES(NULL, '$Name','$Email','$Password' ,'$Phone','$address' ,'$flink','$ilink' ,'$img' ,'false') ";
...
$update=true;
  if(isset($_GET['edit'])){
    $update=false;
    $bid=$_GET['edit'];
    $set= " SELECT * From `local_brand` where `lID` =$bid";
...
if(isset($_POST['update'])){
  ...
  $location = "/xampp/htdocs/printerly/images/".$img;  
    if($_FILES['image']['size']>0){
      $update ="UPDATE `local_brand` SET photo='$img' WHERE `lID`=$bid";
    ...
    elseif($Email == $fetch['l_email']){
      echo "<div style='margin-left: 640px ;' class='mt-2 btn btn-danger'>E-mail already exist</div>";     
    }
else{ 
  $update ="UPDATE `local_brand` SET `l_name` ='$Name' , `l_email`='$Email' , `l_password`='$Password' , `Phone`='$Phone' , `address`='$address'
  , `Flink`='$facebook' , `ilink`='$insta'  WHERE `lID`=$bid";
  $i = mysqli_query($conn ,$update);
...
<?php if($update){ ?>
<h1 class="text-center"> signup as Local Brand</h1>
<?php } ?>
<?php if(!$update){ ?>
      ...
      <div class="mb-3">
       ...
       <div class="mb-3">
        ...
        <div class="mb-3">
          ...
          <input  type="text" value="<?php echo $address; ?>" class="form-control inp" id="exampleFormControlInput1" placeholder="please enter your Location" name="Address">
        </div>
              <div class="mb-3">
